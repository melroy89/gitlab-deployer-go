...
Every ready directory contains the extracted regular files plus `batch.json`, with GitLab identity, commit metadata, artifact SHA-256/size, total extracted size, and a per-file SHA-256 inventory. A downstream consumer may atomically move a directory out of `ready/`; the deployer's durable terminal record intentionally remains for webhook idempotency and restart recovery. Ready batch roots are finalized with mode `0770`. When the consumer has a different host identity, configure a default named ACL for it on `pending/`; that ACL is inherited throughout processing, and the final mode keeps its write permission effective for the atomic claim. Grant the consumer access to `ready/`, but not to `pending/` or `processing/` themselves.

The consumer must copy the claimed batch to publisher-owned storage outside the deployer/container mount, then validate and publish that exact copy. Do not mount signing keys or repository write access into this service.
